function Remove-MailboxOrphaned_SIDs
{
<#
.SYNOPSIS
...
#>
    ...
    Process
    {
		...
		{	        
			...
			$SID_SendAs = $null
			$usrs_access = Get-MailboxPermission $Aliasmbx | ?{($_.isinherited -eq $false) -and -not ($_.User -like “NT AUTHORITY\SELF”)} | Select-Object User, AccessRights
			...
			if($writelog)
			{
				...
				if ($SID_AccessRights -ne $null){$stream.WriteLine($SID_AccessRights)}
				if ($SID_SendAs -ne $null){$stream.WriteLine($SID_SendAs)}
		    	$stream.WriteLine("============================================================================")
			}
		}
    }#End Process
